...
def idInit():
    ...
    songDif = difRead()["songs"]
    for song in songDif:
        nameDic[song["id"]] = song["title_localized"]["default"]
    dicText = json.dumps(nameDic)
    file = open(
        "miho_nt/plugins/Rotaeno_Functions_Collections/untils/id2name.json", "w"
    )
    file.write(dicText)
    file.close()
...
def name2id(name: str) -> str:
    ...
    try:
        ...
        print(f"未发现对应song_id({name}),请检查是否传入合法的歌曲名字！")
        return "UK"
...
def id2name(id: str) -> str:
    ...
    """
    file = open(
        "miho_nt/plugins/Rotaeno_Functions_Collections/untils/id2name.json", "r"
    )
    jsonName = json.load(file)
    file.close()
    try:
        ...
        print(f"未发现对应song_id({id}),请检查是否传入合法的歌曲id！")
        return "UK"
...
def player_options(jsonData: dict) -> tuple:
    """根据玩家数据输出玩家信息

    Args:
        jsonData (dict): 玩家信息json字典

    Returns:
        tuple: 玩家信息组成的元组，分别为背景，角色，勋章，经验
    """
    background = jsonData["results"][0]["cloudSave"]["data"]["data"][
        "collectable-background"
    ]["EquippedBackgroundId"]
    character = jsonData["results"][0]["cloudSave"]["data"]["data"][
        "collectable-character"
    ]["EquippedCharacterId"]
    badges = jsonData["results"][0]["cloudSave"]["data"]["data"]["badges"][
        "EquippedBadgeId"
    ]
    level_xp = jsonData["results"][0]["cloudSave"]["data"]["data"]["PlayerLevel"][
        "AccumXp"
    ]

    return (background, character, badges, level_xp)


def player_details(jsonData: dict) -> tuple:
    """根据玩家数据输出玩家详细信息

    Args:
        jsonData (dict): 玩家数据json字典

    Returns:
        tuple: 玩家详细信息
    """
    playData = jsonData["results"][0]["cloudSave"]["data"]["data"]["playRecords"]
    # print(type(playData))
    playerDetails=[0]*18
    for Data in playData.items():
        if Data[0]=="RecentRatingEntris":
            continue
        if Data[0]=='TotalApp':
            playerDetails[0]=Data[1]
        if Data[0]=='TotalAp':
            playerDetails[1]=Data[1]
        if Data[0]=='TotalFc':
            playerDetails[2]=Data[1]
        if Data[0]=='TotalExPlus':
            playerDetails[3]=Data[1]
        if Data[0]=='TotalEx':
            playerDetails[4]=Data[1]
        if Data[0]=='TotalSPlus':
            playerDetails[5]=Data[1]
        if Data[0]=='TotalS':
            playerDetails[6]=Data[1]
        if Data[0]=='TotalAPlus':
            playerDetails[7]=Data[1]
        if Data[0]=='TotalA':
            playerDetails[8]=Data[1]
        if Data[0]=='TotalB':
            playerDetails[9]=Data[1]
        if Data[0]=='TotalC':
            playerDetails[10]=Data[1]
        if Data[0]=='TotalD':
            playerDetails[11]=Data[1]
        if Data[0]=='TotalE':
            playerDetails[12]=Data[1]
        if Data[0]=='TotalF':
            playerDetails[13]=Data[1]
        if Data[0]=='Tap':
            playerDetails[14]=Data[1]
        if Data[0]=='Catch':
            playerDetails[15]=Data[1]
        if Data[0]=='Flick':
            playerDetails[16]=Data[1]
        if Data[0]=='Rotate':
            playerDetails[17]=Data[1]
    return tuple(playerDetails)
